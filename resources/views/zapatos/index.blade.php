                    ...
                    @foreach ($zapatos as $zapato)
                        <div>
                            ...
                            <div class="flex justify-between items-stretch">
                                <div>
                                    <h3 class="mt-4 text-lg font-semibold text-gray-900">{{ $zapato->denominacion }}</h3>
                                    <p class="mt-1 text-sm font-medium text-gray-600">{{ $zapato->precio }} €</p>
                                </div>
                                <form class="flex content-center mr-4" action="{{ route('carritos.add', $zapato) }}" method="POST">
                                    @csrf
                                    @method('POST')
                                    <button type="submit">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="rgb(17 24 39)" stroke-width="2">
